...
const sampleCoffees = [
  ...
  },
  {
    ...
    description: 'Artisanal sourdough topped with smashed Hass avocado, sea salt, red chili flakes, microgreens, and two poached free-range eggs.',
    shortDescription: 'Crispy sourdough, fresh Hass avocado & poached eggs.',
    price: 349,
    category: 'Breakfast',
    roastLevel: 'N/A',
    origin: 'Fresh Farm Produce',
    flavorNotes: ['Fresh Avocado', 'Chili Flakes', 'Toasted Sourdough'],
    sizes: ['Plate'],
    images: [
  ...
  },
  {
    ...
    description: 'Traditional Italian dessert made with ladyfingers soaked in Brew Haven espresso, layered with whipped mascarpone and cocoa.',
    shortDescription: 'Espresso soaked ladyfingers & mascarpone mousse.',
    price: 279,
    category: 'Desserts',
    roastLevel: 'N/A',
    origin: 'Venice Tradition',
    flavorNotes: ['Mascarpone Cheese', 'Espresso Soak', 'Dutch Cocoa'],
    sizes: ['Slice'],
    images: [
...
const seedDB = async () => {
  try {
    const mongoURI = process.env.MONGODB_URI || 'mongodb://127.0.0.1:27017/brewhaven';
    await mongoose.connect(mongoURI);
    console.log('Connected to MongoDB for seeding...');

    await Coffee.deleteMany({});
    console.log('Cleared existing coffee catalog...');

    const inserted = await Coffee.insertMany(sampleCoffees);
    console.log(`Successfully seeded ${inserted.length} coffee products into Brew Haven database!`);
